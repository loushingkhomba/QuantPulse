...
from src.trade_acceptance import fit_trade_acceptance_model
from src.regime_experts import fit_regime_experts, apply_regime_expert_probs
...
def _safe_logit(prob_values):
    clipped = np.clip(np.asarray(prob_values, dtype=np.float64), 1e-6, 1.0 - 1e-6)
...
def fit_temperature_scaler(raw_probs, labels, min_rows=200):
    """Fit a simple temperature scaler on validation probabilities.

    Uses grid search over temperature T and minimizes Brier score.
    """
    probs = np.asarray(raw_probs, dtype=np.float64)
    y = np.asarray(labels, dtype=np.float64)
    valid_mask = np.isfinite(probs) & np.isfinite(y)
    ...
    if len(probs) < int(min_rows):
        return {
            "enabled": False,
            "reason": f"insufficient_rows:{len(probs)}",
            "temperature": 1.0,
            "pre_brier": None,
            "post_brier": None,
        }
    ...
    temp_grid = np.concatenate([
        np.linspace(0.50, 1.40, 19),
        np.linspace(1.50, 3.00, 16),
    ])
    ...
    pre_brier = float(np.mean((probs - y) ** 2))
    return {
        "enabled": True,
        "reason": "ok",
        "temperature": best_temp,
        "pre_brier": pre_brier,
        "post_brier": best_brier,
    }


def apply_temperature_scaler(raw_probs, temperature):
...
X_train, X_test, y_train, y_test, dates_train, tickers_train_all, tickers_test, dates_test, regime_train, regime_test = prepare_dataset(
    data,
    test_fraction=TEST_FRACTION,
    split_date=split_date_env if split_date_env else None,
    return_regime_state=True,
    return_train_tickers=True,
)

dates_train = pd.Series(pd.to_datetime(dates_train))
dates_test = pd.Series(pd.to_datetime(dates_test))
tickers_train_all = pd.Series(tickers_train_all)
...
tickers_train_all = tickers_train_all.iloc[sort_idx].reset_index(drop=True)
...
confidence_calibration_enabled = os.getenv("QUANT_CONFIDENCE_CALIBRATION_ENABLED", "1").strip() == "1"
confidence_calibration_min_rows = int(os.getenv("QUANT_CONFIDENCE_CALIBRATION_MIN_ROWS", "200"))
confidence_calibration_meta = {"enabled": False, "reason": "disabled"}
...
if signal_mode == "model":
    ...
    validation_gate_history = pd.DataFrame(columns=["ticker", "date", "confidence"])
    ...
    if validation_confidence is not None and len(validation_confidence) == len(val_dates):
        validation_gate_history = pd.DataFrame(
            {
                "ticker": val_tickers.values,
...
if signal_mode == "model" and confidence_calibration_enabled and validation_confidence is not None and len(y_val) > 0:
    calibration_result = fit_temperature_scaler(
        validation_confidence,
        y_val.numpy(),
        min_rows=confidence_calibration_min_rows,
    )
    if calibration_result.get("enabled"):
        temperature = calibration_result["temperature"]
        confidence = apply_temperature_scaler(confidence, temperature)
        validation_confidence = apply_temperature_scaler(validation_confidence, temperature)
        confidence_calibration_meta = {
            "enabled": True,
            "method": "temperature_scaling",
            "temperature": float(temperature),
            "pre_brier": calibration_result.get("pre_brier"),
            "post_brier": calibration_result.get("post_brier"),
        }
        ...
        confidence_calibration_meta = {
            "enabled": False,
            "reason": calibration_result.get("reason", "calibration_failed"),
        }
...
print("Confidence calibration:", confidence_calibration_meta)
...
for optional_col in optional_price_cols:
    if optional_col in data.columns and optional_col not in price_df.columns:
        price_df[optional_col] = data.reset_index()[optional_col]
...
def enrich_signal_frame(signal_frame, apply_missing_noise=False):
    ...
    out.drop(columns=["Date", "Ticker"], inplace=True)
    out = out.dropna(subset=["price"])

    if apply_missing_noise and missing_data_rate > 0:
        missing_rng = np.random.RandomState(SEED + 911)
        keep_mask = missing_rng.rand(len(out)) >= missing_data_rate
    ...
    out = out.groupby(["date", "ticker"], as_index=False).agg(agg_map)

    if confidence_ema_span > 1:
        out = out.sort_values(["ticker", "date"]).reset_index(drop=True)
        out["confidence"] = out.groupby("ticker")["confidence"].transform(
...
pred_df = enrich_signal_frame(pred_df, apply_missing_noise=True)


def _zscore_by_date(date_series, value_series):
    values = pd.to_numeric(value_series, errors="coerce")
    means = values.groupby(date_series).transform("mean")
    stds = values.groupby(date_series).transform("std").replace(0.0, np.nan)
    z_values = (values - means) / (stds + 1e-6)
    return z_values.fillna(0.0)


def build_feature_signal(frame):
    out = frame.copy()
    weights = {
        "trend": float(os.getenv("QUANT_FEATURE_SIGNAL_W_TREND", "0.14")),
        "volatility": float(os.getenv("QUANT_FEATURE_SIGNAL_W_VOLATILITY", "0.10")),
        "drawdown": float(os.getenv("QUANT_FEATURE_SIGNAL_W_DRAWDOWN", "0.10")),
        "rs": float(os.getenv("QUANT_FEATURE_SIGNAL_W_RS", "0.08")),
        "momentum": float(os.getenv("QUANT_FEATURE_SIGNAL_W_MOMENTUM", "0.06")),
        "rsi": float(os.getenv("QUANT_FEATURE_SIGNAL_W_RSI", "0.03")),
    }
    scale = float(os.getenv("QUANT_FEATURE_SIGNAL_SCALE", "0.85"))

    base_logit = _safe_logit(out["confidence"].fillna(0.5).to_numpy())
    adjustment = np.zeros(len(out), dtype=np.float64)

    if "trend_strength" in out.columns:
        adjustment += weights["trend"] * _zscore_by_date(out["date"], out["trend_strength"]).to_numpy()
    if "volatility_regime" in out.columns:
        adjustment -= weights["volatility"] * _zscore_by_date(out["date"], out["volatility_regime"]).to_numpy()
    if "nifty_drawdown_63d" in out.columns:
        adjustment += weights["drawdown"] * _zscore_by_date(out["date"], out["nifty_drawdown_63d"]).to_numpy()
    if "rs_vs_nifty" in out.columns:
        adjustment += weights["rs"] * _zscore_by_date(out["date"], out["rs_vs_nifty"]).to_numpy()
    if "momentum" in out.columns:
        adjustment += weights["momentum"] * _zscore_by_date(out["date"], out["momentum"]).to_numpy()
    if "rsi" in out.columns:
        rsi_centered = pd.to_numeric(out["rsi"], errors="coerce") - 50.0
        adjustment += weights["rsi"] * _zscore_by_date(out["date"], rsi_centered).to_numpy()

    blended = _sigmoid(base_logit + (scale * adjustment))
    out["feature_signal"] = np.clip(blended, 1e-6, 1.0 - 1e-6)

    return out, {
        "enabled": True,
        "scale": scale,
        "weights": weights,
        "signal_mean": float(np.mean(out["feature_signal"])),
        "signal_std": float(np.std(out["feature_signal"])),
    }

gate_fit_base_df = pred_df[
    [
        "ticker",
        "date",
        "confidence",
...
if not validation_gate_history.empty:
    gate_hist_df = enrich_signal_frame(validation_gate_history, apply_missing_noise=False)
    gate_fit_base_df = pd.concat(
        [
            gate_hist_df[
                [
                    "ticker",
    ...
    gate_fit_base_df = gate_fit_base_df.sort_values(["date", "ticker"]).drop_duplicates(
...
feature_signal_enabled = os.getenv("QUANT_FEATURE_SIGNAL_ENABLED", "1").strip() == "1"
feature_signal_meta = {"enabled": False, "reason": "disabled"}
real_signal_col = "confidence"
if feature_signal_enabled:
    pred_df, feature_signal_meta = build_feature_signal(pred_df)
    pred_df["inv_feature_signal"] = 1.0 - pred_df["feature_signal"]
    real_signal_col = "feature_signal"
...
trade_acceptance_enabled = os.getenv("QUANT_TRADE_ACCEPTANCE_ENABLED", "0").strip() == "1"
trade_acceptance_threshold_override = os.getenv("QUANT_TRADE_ACCEPTANCE_THRESHOLD", "").strip()
trade_acceptance_epochs = int(os.getenv("QUANT_TRADE_ACCEPTANCE_EPOCHS", "25"))
trade_acceptance_min_rows = int(os.getenv("QUANT_TRADE_ACCEPTANCE_MIN_ROWS", "800"))
regime_experts_enabled = os.getenv("QUANT_REGIME_EXPERTS_ENABLED", "0").strip() == "1"
regime_experts_min_rows = int(os.getenv("QUANT_REGIME_EXPERTS_MIN_ROWS", "300"))
flip_risk_enabled = os.getenv("QUANT_FLIP_RISK_ENABLED", "0").strip() == "1"
flip_risk_threshold_override = os.getenv("QUANT_FLIP_RISK_THRESHOLD", "").strip()
flip_risk_epochs = int(os.getenv("QUANT_FLIP_RISK_EPOCHS", "20"))
flip_risk_min_rows = int(os.getenv("QUANT_FLIP_RISK_MIN_ROWS", "600"))
...
def _parse_float_list(env_name, default_csv):
...
def _parse_int_list(env_name, default_csv):
    raw = os.getenv(env_name, default_csv)
    values = []
    for token in str(raw).split(","):
        token = token.strip()
...
def _downsample_signal_frame(frame, drop_rate, seed):
    rate = float(min(max(drop_rate, 0.0), 0.95))
    if rate <= 0.0:
        return frame.copy()
    rng = np.random.RandomState(seed)
    keep_mask = rng.rand(len(frame)) >= rate
    return frame[keep_mask].copy().reset_index(drop=True)
...
gate_fit_df, _, _ = build_forward_metric_columns(
    gate_fit_base_df,
    horizons=HORIZON_DAYS,
    execution_delay_days=execution_delay,
)

pred_df["trade_accept_prob"] = 1.0
trade_acceptance_active_threshold = 0.5
trade_acceptance_meta = {"enabled": False, "reason": "disabled"}
if trade_acceptance_enabled and test_window_start is not None:
    trade_acceptance_result = fit_trade_acceptance_model(
        gate_fit_df,
        cutoff_date=test_window_start,
        epochs=trade_acceptance_epochs,
        min_rows=trade_acceptance_min_rows,
        seed=SEED,
    )
    if trade_acceptance_result.get("trained"):
        gate_fit_df = gate_fit_df.copy()
        gate_fit_df["trade_accept_prob"] = trade_acceptance_result["probs"]
        gate_prob_map = gate_fit_df[["date", "ticker", "trade_accept_prob"]].drop_duplicates(
            ["date", "ticker"], keep="last"
        ).rename(columns={"trade_accept_prob": "trade_accept_prob_gate"})
        ...
        trade_acceptance_meta = {
            ...
            "train_rows": trade_acceptance_result.get("train_rows"),
            "val_rows": trade_acceptance_result.get("val_rows"),
            "val_loss": trade_acceptance_result.get("val_loss"),
        }
    else:
        trade_acceptance_meta = {
            "enabled": False,
            "reason": trade_acceptance_result.get("reason", "training_failed"),
        }

# Phase-2: regime-aware expert gate
regime_experts_meta = {"enabled": False, "reason": "disabled"}
if regime_experts_enabled and test_window_start is not None:
    _re_results = fit_regime_experts(
        gate_fit_df,
        cutoff_date=test_window_start,
        epochs=trade_acceptance_epochs,
        min_rows_per_regime=regime_experts_min_rows,
        seed=SEED,
    )
    _any_re_trained = any(v.get("trained") for v in _re_results.values())
    if _any_re_trained:
        pred_df = apply_regime_expert_probs(
            _re_results,
            pred_df,
            fallback_threshold=trade_acceptance_active_threshold,
        )
        regime_experts_meta = {
            "enabled": True,
            "experts": {
                k: {
                    ...
                    "val_rows": v.get("val_rows"),
                    "reason": v.get("reason"),
                }
                for k, v in _re_results.items()
            },
        }
    else:
        regime_experts_meta = {"enabled": False, "reason": "no_regime_had_sufficient_rows"}

pred_df["flip_risk_prob"] = 0.0
flip_risk_active_threshold = 1.0
flip_risk_meta = {"enabled": False, "reason": "disabled"}
if flip_risk_enabled and test_window_start is not None:
    flip_risk_result = fit_flip_risk_model(
        gate_fit_df,
        cutoff_date=test_window_start,
        epochs=flip_risk_epochs,
        min_rows=flip_risk_min_rows,
        seed=SEED,
    )
    if flip_risk_result.get("trained"):
        gate_fit_df = gate_fit_df.copy()
        gate_fit_df["flip_risk_prob"] = flip_risk_result["probs"]
        flip_prob_map = gate_fit_df[["date", "ticker", "flip_risk_prob"]].drop_duplicates(
            ["date", "ticker"], keep="last"
        ).rename(columns={"flip_risk_prob": "flip_risk_prob_gate"})
        pred_df = pred_df.merge(flip_prob_map, on=["date", "ticker"], how="left")
        pred_df["flip_risk_prob"] = pred_df["flip_risk_prob_gate"].fillna(0.0)
        pred_df.drop(columns=["flip_risk_prob_gate"], inplace=True)

        if flip_risk_threshold_override:
            flip_risk_active_threshold = float(flip_risk_threshold_override)
        else:
            flip_risk_active_threshold = float(flip_risk_result["threshold"])

        flip_risk_meta = {
            "enabled": True,
            "threshold": flip_risk_active_threshold,
            "train_rows": flip_risk_result.get("train_rows"),
            "val_rows": flip_risk_result.get("val_rows"),
            "val_loss": flip_risk_result.get("val_loss"),
        }
    else:
        flip_risk_meta = {
            "enabled": False,
            "reason": flip_risk_result.get("reason", "training_failed"),
        }
...
print("Feature signal layer:", feature_signal_meta)
...
def run_backtest(
    conf_col,
    apply_kill_switch=None,
    signal_frame=None,
    transaction_cost_override=None,
    slippage_override=None,
    max_new_positions_override=None,
):
    """
    ...
    effective_slippage = slippage if slippage_override is None else float(slippage_override)
    effective_max_new_positions = (
        turnover_min_new_positions
        if max_new_positions_override is None
        else int(max(0, max_new_positions_override))
    )
    ...
    for i in range(max_iter):
        ...
        day_max_new_positions = effective_max_new_positions
        ...
        for idx, row in top.reset_index(drop=True).iterrows():
            ...
            entry_price = today_price * (1 + effective_slippage)
            exit_price = next_price[0] * (1 - effective_slippage)
            ...
            r -= (2 * effective_transaction_cost)
            ...
            day_records.append(
                {
                    ...
                    "acceptance_mean": float(top["trade_accept_prob"].mean()) if "trade_accept_prob" in top.columns else None,
...
def run_robustness_sensitivity(base_signal_col="confidence"):
    enabled = os.getenv("QUANT_ROBUSTNESS_ENABLED", "1").strip() == "1"
    if not enabled:
        return {
            "enabled": False,
            "reason": "disabled",
    ...
    min_final_ratio = float(os.getenv("QUANT_ROBUSTNESS_MIN_FINAL_RATIO", "0.90"))
    min_sharpe_delta = float(os.getenv("QUANT_ROBUSTNESS_MIN_SHARPE_DELTA", "-0.40"))
    cost_multipliers = _parse_float_list("QUANT_ROBUSTNESS_COST_MULTIPLIERS", "1.5,2.0")
    slippage_multipliers = _parse_float_list("QUANT_ROBUSTNESS_SLIPPAGE_MULTIPLIERS", "1.5,2.0")
    missing_rates = _parse_float_list("QUANT_ROBUSTNESS_MISSING_RATES", "0.10,0.20")
    max_new_positions_values = _parse_int_list("QUANT_ROBUSTNESS_MAX_NEW_POSITIONS_VALUES", "1")

    base_final, base_sharpe, base_dd, _, _ = run_backtest(base_signal_col)

    scenarios = [
        {
            "name": "base",
            "kind": "baseline",
            "final": float(base_final),
            "sharpe": float(base_sharpe),
            "max_drawdown_pct": float(base_dd * 100.0),
            "final_ratio_vs_base": 1.0,
            "sharpe_delta_vs_base": 0.0,
            "pass": True,
        }
    ]

    for multiplier in cost_multipliers:
        scenario_name = f"cost_x{multiplier:.2f}"
        final_v, sharpe_v, dd_v, _, _ = run_backtest(
            base_signal_col,
            transaction_cost_override=transaction_cost * multiplier,
        )
        final_ratio = _safe_ratio(final_v, base_final)
        sharpe_delta = None if base_sharpe is None else float(sharpe_v - base_sharpe)
        passed = (
            final_ratio is not None
            and sharpe_delta is not None
            and final_ratio >= min_final_ratio
        ...
        scenarios.append(
            {
                ...
                "max_drawdown_pct": float(dd_v * 100.0),
                "final_ratio_vs_base": final_ratio,
    ...
    for multiplier in slippage_multipliers:
        scenario_name = f"slippage_x{multiplier:.2f}"
        final_v, sharpe_v, dd_v, _, _ = run_backtest(
            base_signal_col,
            slippage_override=slippage * multiplier,
        )
        final_ratio = _safe_ratio(final_v, base_final)
        sharpe_delta = None if base_sharpe is None else float(sharpe_v - base_sharpe)
        passed = (
    ...
    for idx, drop_rate in enumerate(missing_rates):
        scenario_name = f"missing_{int(round(drop_rate * 100))}pct"
        stressed_frame = _downsample_signal_frame(pred_df, drop_rate=drop_rate, seed=SEED + 1700 + idx)
        final_v, sharpe_v, dd_v, _, _ = run_backtest(
            base_signal_col,
            signal_frame=stressed_frame,
        )
        final_ratio = _safe_ratio(final_v, base_final)
        sharpe_delta = None if base_sharpe is None else float(sharpe_v - base_sharpe)
        passed = (
        ...
        scenarios.append(
            {
                "name": scenario_name,
                "kind": "missing_data_stress",
                "drop_rate": float(drop_rate),
                "rows_kept": int(len(stressed_frame)),
                "rows_base": int(len(pred_df)),
                "final": float(final_v),
                "sharpe": float(sharpe_v),
                "max_drawdown_pct": float(dd_v * 100.0),
                "final_ratio_vs_base": final_ratio,
    ...
    for max_new_positions in max_new_positions_values:
        scenario_name = f"max_new_positions_{int(max_new_positions)}"
        final_v, sharpe_v, dd_v, _, _ = run_backtest(
            base_signal_col,
            max_new_positions_override=max_new_positions,
        )
        final_ratio = _safe_ratio(final_v, base_final)
        sharpe_delta = None if base_sharpe is None else float(sharpe_v - base_sharpe)
        passed = (
        ...
        scenarios.append(
            {
                "name": scenario_name,
                "kind": "turnover_constraint_stress",
    ...
    non_base = [s for s in scenarios if s.get("kind") != "baseline"]
    overall_pass = all(bool(s.get("pass")) for s in non_base) if non_base else True
    failing = [s["name"] for s in non_base if not s.get("pass")]
    ...
    robustness_dir = Path("logs") / "robustness_checks"
    robustness_dir.mkdir(parents=True, exist_ok=True)
    file_tag_start = str(report["window"].get("start") or "na").replace("-", "")
    file_tag_end = str(report["window"].get("end") or "na").replace("-", "")
    report_path = robustness_dir / f"robustness_{file_tag_start}_{file_tag_end}.json"
    report_path.write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
    (robustness_dir / "latest.json").write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
...
def build_standard_metrics_artifact(real_stats, signal_col="confidence"):
    ...
    return {
        ...
        "feature_signal_layer": feature_signal_meta,
...
def _load_walkforward_history(history_path):
...
def _load_policy_history(history_path):
    if not history_path.exists():
        return []
    try:
        payload = json.loads(history_path.read_text(encoding="utf-8"))
...
def evaluate_promotion_policy(artifact, wf_report, robustness_report):
    min_final_ratio = float(os.getenv("QUANT_POLICY_MIN_FINAL_RATIO", "1.03"))
    min_sharpe_edge = float(os.getenv("QUANT_POLICY_MIN_SHARPE_EDGE", "0.0"))
    min_annualized_return_pct = float(os.getenv("QUANT_POLICY_MIN_ANNUALIZED_RETURN_PCT", "3.0"))
    require_wf_pass = os.getenv("QUANT_POLICY_REQUIRE_WF_PASS", "1").strip() == "1"
    ...
    final_value = _safe_float(backtest.get("final_value"))
    random_final = _safe_float(backtest.get("random_final"))
    sharpe = _safe_float(backtest.get("sharpe"))
    annualized_return_pct = _safe_float(backtest.get("annualized_return_pct"))

    random_sharpe_local = _safe_float(rand_sharpe)
    final_ratio = _safe_ratio(final_value, random_final)
    sharpe_edge = None
    if sharpe is not None and random_sharpe_local is not None:
        sharpe_edge = sharpe - random_sharpe_local

    wf_pass = bool(wf_report.get("overall_pass")) if isinstance(wf_report, dict) else False
    robustness_pass = bool(robustness_report.get("overall_pass")) if isinstance(robustness_report, dict) else False

    checks = {
        "edge_final_ratio": final_ratio is not None and final_ratio >= min_final_ratio,
        "edge_sharpe": sharpe_edge is not None and sharpe_edge >= min_sharpe_edge,
        "annualized_return": annualized_return_pct is not None and annualized_return_pct >= min_annualized_return_pct,
        "wf_gate": (wf_pass if require_wf_pass else True),
        "robustness_gate": (robustness_pass if require_robustness_pass else True),
    }
    candidate_promote = all(checks.values())

    hard_rollback_reasons = []
    if rollback_on_wf_fail and not wf_pass:
        hard_rollback_reasons.append("wf_fail")
    if rollback_on_robustness_fail and not robustness_pass:
        hard_rollback_reasons.append("robustness_fail")
    candidate_rollback = len(hard_rollback_reasons) > 0

    history_dir = Path("logs") / "promotion_policy"
    history_dir.mkdir(parents=True, exist_ok=True)
    history_path = history_dir / "history.json"
    history_entries = _load_policy_history(history_path)

    promote_streak = 1 if candidate_promote else 0
    rollback_streak = 1 if candidate_rollback else 0
    if history_entries:
        last = history_entries[-1]
        if candidate_promote:
            promote_streak = int(last.get("promote_streak", 0)) + 1
        if candidate_rollback:
            rollback_streak = int(last.get("rollback_streak", 0)) + 1

    if rollback_streak >= rollback_streak_required:
        decision = "rollback"
    elif promote_streak >= promote_streak_required:
        decision = "promote"
    else:
        decision = "hold"
    ...
    reasons = []
    if decision == "promote":
        reasons.append("promote_streak_reached")
    elif decision == "rollback":
        reasons.extend(hard_rollback_reasons or ["rollback_streak_reached"])
    else:
        for check_name, check_ok in checks.items():
            if not check_ok:
                reasons.append(f"failed_{check_name}")
        if not reasons:
    ...
    report = {
        ...
        "thresholds": {
            ...
            "require_wf_pass": require_wf_pass,
            "require_robustness_pass": require_robustness_pass,
        },
        "observed": {
            "final_ratio_vs_random": final_ratio,
            "sharpe_edge_vs_random": sharpe_edge,
    ...
    file_tag_start = str(window.get("start") or "na").replace("-", "")
    file_tag_end = str(window.get("end") or "na").replace("-", "")
    report_path = history_dir / f"policy_{file_tag_start}_{file_tag_end}.json"
    report_path.write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
    (history_dir / "latest.json").write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
    ...
    history_path.write_text(json.dumps(_json_safe(history_entries), indent=2), encoding="utf-8")

    return report, report_path


def evaluate_walkforward_governance(artifact):
    min_sharpe = float(os.getenv("QUANT_WF_MIN_SHARPE", "0.0"))
    max_drawdown_pct = float(os.getenv("QUANT_WF_MAX_DRAWDOWN_PCT", "35.0"))
    max_avg_turnover_changed = float(os.getenv("QUANT_WF_MAX_TURNOVER_CHANGED", "2.5"))
    min_baseline_final_ratio = float(os.getenv("QUANT_WF_MIN_BASELINE_FINAL_RATIO", "1.02"))
    min_baseline_sharpe_edge = float(os.getenv("QUANT_WF_MIN_BASELINE_SHARPE_EDGE", "0.0"))
    max_ic_drop_fraction = float(os.getenv("QUANT_WF_MAX_IC_DROP_FRACTION", "0.50"))

    backtest = artifact.get("backtest_summary", {})
    turnover = artifact.get("portfolio_metrics", {}).get("turnover_rate", {})
    ic_metrics = artifact.get("signal_quality", {}).get("information_coefficient", {})
    window = artifact.get("window", {})

    real_sharpe = _safe_float(backtest.get("sharpe"))
    max_dd_pct = _safe_float(backtest.get("max_drawdown_pct"))
    real_final = _safe_float(backtest.get("final_value"))
    random_final = _safe_float(backtest.get("random_final"))
    avg_changed = _safe_float(turnover.get("average_changed_positions"))
    ic_mean = _safe_float(ic_metrics.get("mean_daily_ic"))

    final_ratio = _safe_ratio(real_final, random_final)

    random_sharpe = _safe_float(rand_sharpe)
    sharpe_edge = None
    if real_sharpe is not None and random_sharpe is not None:
        sharpe_edge = real_sharpe - random_sharpe

    history_dir = Path("logs") / "walkforward_gates"
    history_dir.mkdir(parents=True, exist_ok=True)
    history_path = history_dir / "history.json"
    history_entries = _load_walkforward_history(history_path)

    prev_ic = None
    if history_entries:
        prev_ic = _safe_float(history_entries[-1].get("ic_mean"))

    ic_drop_fraction = None
    if ic_mean is not None and prev_ic is not None and abs(prev_ic) > 1e-9:
        ic_drop_fraction = (prev_ic - ic_mean) / abs(prev_ic)

    gate_checks = {
        "sharpe_gate": (real_sharpe is not None and real_sharpe >= min_sharpe),
        "drawdown_gate": (max_dd_pct is not None and abs(max_dd_pct) <= max_drawdown_pct),
        "turnover_gate": (avg_changed is not None and avg_changed <= max_avg_turnover_changed),
        "baseline_delta_gate": (
            final_ratio is not None
            and sharpe_edge is not None
            and final_ratio >= min_baseline_final_ratio
            and sharpe_edge >= min_baseline_sharpe_edge
        ),
        "ic_stability_gate": (
            ic_drop_fraction is None or ic_drop_fraction <= max_ic_drop_fraction
        ),
    }
    ...
    report = {
        ...
        "thresholds": {
            ...
            "max_avg_turnover_changed": max_avg_turnover_changed,
            "min_baseline_final_ratio": min_baseline_final_ratio,
            "min_baseline_sharpe_edge": min_baseline_sharpe_edge,
            "max_ic_drop_fraction": max_ic_drop_fraction,
        },
        "observed": {
            "real_sharpe": real_sharpe,
            "max_drawdown_pct": max_dd_pct,
            "avg_turnover_changed": avg_changed,
            "final_ratio_vs_random": final_ratio,
            "sharpe_edge_vs_random": sharpe_edge,
            "ic_mean": ic_mean,
            "previous_ic_mean": prev_ic,
            "ic_drop_fraction": ic_drop_fraction,
        },
    ...
    file_tag_start = str(window.get("start") or "na").replace("-", "")
    file_tag_end = str(window.get("end") or "na").replace("-", "")
    report_path = history_dir / f"wf_gate_{file_tag_start}_{file_tag_end}.json"
    report_path.write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
    (history_dir / "latest.json").write_text(json.dumps(_json_safe(report), indent=2), encoding="utf-8")
...
real_final, real_sharpe, real_dd, real_curve, real_stats = run_backtest(real_signal_col)
active_signal_col = real_signal_col
...
inv_signal_col = "inv_feature_signal" if active_signal_col == "feature_signal" else "inv_confidence"
inv_final, inv_sharpe, inv_dd, inv_curve, inv_stats = run_backtest(inv_signal_col)
# -------------------------------------------------
...
wf_gate_report, wf_gate_path = evaluate_walkforward_governance(standard_metrics_artifact)
robustness_report, robustness_path = run_robustness_sensitivity(active_signal_col)
policy_report, policy_path = evaluate_promotion_policy(
    standard_metrics_artifact,
    wf_gate_report,
    robustness_report,
)
print_standard_metrics_summary(standard_metrics_artifact)
print("Standard metrics saved:", standard_metrics_path)
print("Walk-forward governance:", wf_gate_report["gate_checks"], "pass=", wf_gate_report["overall_pass"])
print("Walk-forward gate report saved:", wf_gate_path)
if robustness_report.get("enabled"):
    print(
        "Robustness sensitivity:",
        {
            "overall_pass": robustness_report.get("overall_pass"),
            "scenario_count": robustness_report.get("scenario_count"),
            "failing_scenarios": robustness_report.get("failing_scenarios"),
        },
    )
    print("Robustness report saved:", robustness_path)
else:
    print("Robustness sensitivity:", robustness_report.get("reason", "disabled"))
print(
    "Promotion policy:",
    {
        "decision": policy_report.get("decision"),
        "reasons": policy_report.get("reasons"),
        "promote_streak": policy_report.get("promote_streak"),
        "rollback_streak": policy_report.get("rollback_streak"),
    },
)
print("Promotion policy report saved:", policy_path)
